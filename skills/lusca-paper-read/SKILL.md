...
6. **交付**：笔记路径 + 一两句简要说明（论文一句话定位 + verdict）；**不内联重复展示完整笔记**——完整内容已落盘，再贴一遍是冗余噪声
...
- **列表符号规范：并列用无序、有递进才用有序、不堆 ①②③**：并列要点一律用无序列表（`-`）逐行展开，**不要把 ①②③ 塞在同一行**——要用 ① 分点就分行写成列表项，或直接用无序 `-`；有序列表（`1.` `2.`）仅用于真正有先后/递进/步骤关系的内容，**无递进的并列（多条贡献、多个假设、多个发现、多个警惕点）用无序不用有序**
