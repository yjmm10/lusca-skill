...
### Changed
- 文末出处块由"一行三要素（｜ 分隔）"改为"**三行引用块**"（每行一个要素：作者 / 版本 / 出处），提升扫读可读性；`SKILL.md` 出处块说明同步（遵 CLAUDE.md「文档输出规范」）

## [1.10.0] — 2026-07-22

### Added
- 新增 `references/decoupling-and-format.md`（论文结构解耦与格式规范），把本会话 UAV 论文多轮润色中反复暴露的**通用结构问题**固化为跨节硬纪律（来源：用户反馈"提取对通用论文需注意的内容，确保论文解耦、表达符合论文要求"）：
  - **理论章节与实验/代码解耦**：建模/系统模型/问题形式化只讲通用变量与理想场景，禁带实验取值（$B=1$、$n=18$）、实验处理方式（种子/对比实验/在线离线）、代码引用（见开源仓库）、算法提前绑定（理论就写"用 QMIX"）、实验预演表述、章节预告；并强调"在线推断 vs 提前标注"要说清触发时机。
  - **符号与公式出场纪律**：首次定义处才用、引言/相关工作/贡献清单不嵌公式、派生量与冗余符号清理。
  - **图与表**：图说只留基本说明（删内容描述/占位符）；参数表归实验章节；表注≠正文≠脚注（不用 markdown `>` 充当）；无增量对照表删。
  - **方法命名一致性**：整体方法给统摄名，不拼子模块缩写（SATCPP-UAR→SUAC）；全文同步。
  - **术语领域惯例**：通用译法优先（实验臂→实验组），首现给全称+缩写。
  - **去结构冗余**：删目录式章节导引、清单硬编号（其一其二其三→连贯论述）、赘述总结句、自我标榜收束。
- SKILL.md 新增「结构与格式卫生（跨节）」节指向该 reference；Reference 索引表加行。

### Root Cause（记入 CHANGELOG 以备回溯）
- 本会话对 UAV 论文做"理论↔实验解耦 + 命名统一 + 术语替换"大改时，发现这些问题是**跨节的、通用的结构卫生**问题，而 polish 此前的 references（section-conventions 讲各节内容职责、paradigms 讲各节组织范式、ai-tone-guardrails 讲词级 AI 腔）均未专门覆盖。逐节润色易只改局部表达、漏掉"理论里混进实验取值""引言嵌公式""图说写成内容描述""模块缩写拼接当方法名"这类跨节硬伤。本次以独立 reference 固化判定口径与实例，与现有 references 正交互补。

## [1.9.0] — 2026-07-19

### Added
- SKILL.md「输出与存放」新增两条产出洁净度规范（来源：用户反馈"润色之后的论文不能出现本地仓库或具体代码位置，要自解耦；占位符要告知、补全后清理" + UAV 论文 final frontmatter `data_source` 残留本地 JSON 路径的实战）：
  - **润色产出不得残留本地仓库/代码位置（自解耦原则）**：落盘润色稿（尤其可投稿定稿）须自解耦，清除本地路径（`results/*.json`、`src/...`、`/code/...`）、本地文件名、指向本地的开发元字段；**图表路径例外**（`figures/*.png` 保留）；交付前 grep 复查。明确此约束**只针对论文文档**——本地代码仓库本身（训练产物、src、checkpoint）一律不动（研究资产 + 复现性声明挂钩）。
  - **占位符要告知、补全后及时清理**：遇 `[仓库地址待补充]` 等占位符，交付时列"待补清单"告知用户；用户提供后及时替换复查，不默默保留进定稿。
- SKILL.md「重要约定」新增速查条目"润色产出自解耦"。

### Root Cause（记入 CHANGELOG 以备回溯）
- 本会话先误读用户意图（以为要删本地代码仓库）、正确拒绝后又获澄清：用户要的是清**理论文文档里**的本地数据引用，不是删本地仓库。暴露 polish 此前对"产出物洁净度"无显式规范——final 可投稿稿可能残留 `results/*.json` 这类本地路径，占位符也无管理流程。本次以两条规范 + 速查条目补齐，并固化"清理论文文档 vs 不动本地仓库"的职责边界。

## [1.8.0] — 2026-07-18

### Changed
- `assets/paradigms/README.md`「通篇横向维度（2 条）」**升级为「整篇体检：全局一致性核查表」**（来源：用户反馈"为什么 polish 发现不了整篇问题" + UAV 论文整篇审稿排查实例）：把逐节润色易漏的跨节硬伤固化为 **9 项反向核查**（跨节数字/参数一致、公式量纲一致、符号首现定义、术语跨节统一、表/图编号体系、引用闭环、图表互补不重复、文字跨节不重复、章节衔接、内容严谨标注不修），每项配本会话实例；并在「定位」明确 **README = 全局体检层 / 各节 `<节>.md` = 局部优化层** 的分工。
- SKILL.md「主流程」新增 **Step 5「整篇体检」**：长文交付前 `Read` 全文按核查表横向核查、输出问题清单（只标注不擅改，数据/公式/推导类标"待作者确认"），与逐段润色互补；短文可略。

### Root Cause（记入 CHANGELOG 以备回溯）
- 本会话暴露的结构性短板：polish 原为"逐节润色"设计，强在局部表达、弱在全局一致性；paradigms 的维度是"单节正向指导"（怎么写好），缺"全局反向核查"（查什么错），且流程无"整篇通读体检"步骤。本次以 README 核查表 + Step 5 补齐——polish 从"润色"扩展为"润色 + 整篇体检"。
