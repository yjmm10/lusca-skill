...
| 已有初稿，润色论文表达与结构 | `lusca-paper-polish` |
...
**不触发**：用户只是要查单个已知 DOI/URL 的详情（直接 WebFetch 即可），或在 `lusca-paper-polish` 润色流程中引用已有文献（无需重新检索）。
...
1. **跨源标题去重**：标题归一化（小写、去标点、压缩空白）后相同者合并为一条，
   **保留最优版本**——**DBLP 优先级最低**（DBLP 无原生摘要、信息不足，重复组里只要有
   非 dblp 源就丢弃 dblp 版本）；非 dblp 之间再按字段完整度加权取最全（摘要 3、URL 2、
   其余元数据各 1；`citation_count=0` 视为有效字段）。合并后的记录在其主来源下展示，并以
   `sources` 字段记录所有命中来源（含 dblp）。无标题的论文不参与去重，原样保留。
...
检索导览 + 索引落盘后，交付时附一句最相关的下一步。本技能是「检索→解析→精读」主链的**起点**：

- **`/lusca-pdf-parse`**（最常见，主链下一步）：把「推荐阅读」挑出的那篇 PDF 解析成干净 Markdown——复杂版面（双栏 / 公式密集 / 表格）不乱、图不丢
- 解析完再交 **`/lusca-paper-read`** 基于该 Markdown 精读（完整链路：检索→解析→精读，比直接读 PDF 更准、图不丢）
- **`/lusca`**：复盘本次检索中 skill 暴露的问题（检索词构造、来源选择、去重 / 导览质量）

> 话术示例：报告路径 + 要点后补一句「挑出想深读的那篇，先 `/lusca-pdf-parse` 解析成 Markdown，再 `/lusca-paper-read` 精读（比直接读 PDF 更准、图不丢）」。
